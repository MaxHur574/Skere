  ...
  >
    <div class="inv-app">
      ...
      <div class="file-list">
        ...
        <Transition name="file-appear">
          ...
          >
            ...
            <div class="file-info">
              <div class="file-name file-name-special">INFORME_POLICIAL_COMPLETO.val</div>
              <div class="file-sub file-sub-special">Desbloqueado · doble clic para leer</div>
            </div>
          </div>
        </Transition>
      </div>
      ...
      <div class="file-preview">
        ...
        <div v-else-if="selected === 'informe'" class="preview-info">
          ...
          <div class="pi-row"><span>Estado</span><span>🟢 Desbloqueado</span></div>
          <div class="pi-row"><span>Tipo</span><span>.val — archivo cifrado</span></div>
          <div class="pi-row"><span>Autor</span><span>E.V.</span></div>
          <div class="pi-hint pi-hint-special">Doble clic para leer el informe.</div>
        </div>
        <div v-else class="preview-info">
          <div class="pi-name">{{ currentNota?.filename }}</div>
          <div class="pi-row">
            <span>Estado</span>
            <span>{{ mg.unlocked[currentNota?.key] ? '🟢 Desbloqueado' : '🔴 Bloqueado' }}</span>
          </div>
          <div class="pi-row"><span>Tipo</span><span>.val — archivo cifrado</span></div>
          <div class="pi-row"><span>Autor</span><span>E.V.</span></div>
          <div class="pi-hint">
    ...
    <Teleport to="body">
      <Transition name="fade-in">
        <div v-if="showInforme" class="informe-overlay" @click.self="showInforme = false">
          <div class="informe-window">
            <div class="informe-header">
              <span class="informe-title-bar">📋 INFORME_POLICIAL_COMPLETO.val</span>
              <button class="informe-close" @click="showInforme = false">✕</button>
            </div>
            <div class="informe-body">
              <div class="informe-stamp">POLICIA — USO INTERNO — CLASIFICADO</div>
              <div class="informe-divider">══════════════════════════════════════</div>
              ...
              <div class="informe-field">
                <span class="ifield-label">Primera desaparición confirmada</span>
                <span class="ifield-val ifield-red">14/10/2020 — 03:14 AM</span>
              </div>

              <div class="informe-divider informe-divider-sm">──────────────────────────</div>

              <div class="informe-block">
                Desde entonces se registran <span class="highlight">47 desapariciones</span> relacionadas al mismo patrón.
              </div>
              <div class="informe-block">
                La fecha <span class="highlight">14/10/2020</span> aparece repetida en todos los registros.
              </div>
              <div class="informe-block">
                La hora <span class="highlight">03:14</span> coincide en: cámaras, actividad del sistema,
                accesos no autorizados, evidencia recuperada.
              </div>
              <div class="informe-block">
                La figura registrada a las 03:14 <span class="ifield-red">no aparece en ninguna base oficial.</span>
              </div>
              <div class="informe-block">
                Los expedientes posteriores fueron <span class="ifield-red">alterados manualmente.</span>
              </div>

              <div class="informe-divider">══════════════════════════════════════</div>
              <div class="informe-fin">[ FIN DEL DOCUMENTO — NIVEL DE ACCESO: MÁXIMO ]</div>
            </div>
          ...
          <div class="terminal-window">
            ...
            <div class="terminal-body" ref="terminalBodyRef">
              ...
              <div v-if="showKeyInput" class="term-input-row">
                ...
                />
                <button class="term-enter-btn" @click="tryTerminalKey">↵</button>
              </div>
...
const TERMINAL_INTRO_LINES = [
  ...
  { text: '', type: 'ok' },
  { text: '> Para continuar se requiere autenticación de nivel máximo.', type: 'log' },
  { text: '', type: 'ok' },
];
...
watch(allUnlocked, (v) => {
  ...
  system.triggerEvent('notification', { message: 'Nuevo archivo desbloqueado: INFORME_POLICIAL_COMPLETO.val' });
});
...
function tryTerminalKey() {
  ...
  if (entered === SECRET_KEY) {
    ...
    nextTick(() => keyInputRef.value?.focus());
    setTimeout(() => { keyError.value = false; }, 2500);
  }
}
</script>
...
.file-item-special:hover { background: #151000; }
.file-item-special.selected { background: #1a1200; }

@keyframes pulse-row {
  0%, 100% { background: #0d0800; }
  50%       { background: #1a1000; }
}
...
.preview-empty { color: #2a2a2a; font-size: 12px; text-align: center; }

.preview-info { padding: 24px; width: 100%; }
.pi-name {
...
.pi-hint { margin-top: 16px; font-size: 11px; color: #3a5a3a; font-style: italic; }
.pi-hint-special { color: #664422 !important; }

/* Transición de aparición del informe */
.file-appear-enter-active { transition: all 0.5s ease; }
.file-appear-enter-from   { opacity: 0; transform: translateY(-6px); }
...
.informe-title-bar { font-size: 12px; color: #cc8844; font-family: 'Courier New', monospace; }
.informe-close {
  background: none; border: none; color: #553300;
  cursor: pointer; font-size: 13px; transition: color 0.15s;
}
.informe-close:hover { color: #ff4444; }
...
.informe-stamp {
  font-size: 10px; color: #553300;
  letter-spacing: 2px; text-align: center;
}
.informe-divider { color: #221100; font-size: 12px; }
.informe-divider-sm { color: #1a0d00; font-size: 11px; }

.informe-section-title {
  font-size: 14px; color: #aa6622;
  font-weight: bold; letter-spacing: 2px; text-align: center;
}

.informe-field {
  display: flex; flex-direction: column; gap: 2px;
  padding: 6px 0; border-bottom: 1px solid #110800;
}
.ifield-label { font-size: 9px; color: #443300; letter-spacing: 1px; text-transform: uppercase; }
.ifield-val   { font-size: 13px; color: #998855; }
.ifield-red   { color: #cc3333 !important; }

.informe-block {
  font-size: 12px; color: #776644; line-height: 1.8;
}
.highlight { color: #cc8844; font-weight: bold; }

.informe-fin {
  font-size: 10px; color: #332200;
  text-align: center; letter-spacing: 1px; margin-top: 8px;
}
...
.terminal-dot.red    { background: #cc3333; box-shadow: 0 0 6px #cc3333; }
.terminal-dot.yellow { background: #cc8800; }
...
.term-line.ok       { color: #226622; }
.term-line.warn     { color: #aa7700; }
.term-line.err      { color: #cc3333; }
.term-line.dim      { color: #222; }
.term-line.title    { color: #cc3333; font-size: 14px; font-weight: bold; letter-spacing: 2px; }
.term-line.log      { color: #888; }
.term-line.success  { color: #33cc66; font-weight: bold; }
.term-line.progress { color: #cc3333; letter-spacing: 1px; font-size: 12px; }
...
.term-input::placeholder { color: #330000; letter-spacing: 2px; }
...
.fade-in-enter-active { transition: opacity 0.35s ease; }
.fade-in-enter-from   { opacity: 0; }
.fade-in-leave-active { transition: opacity 0.25s ease; }
