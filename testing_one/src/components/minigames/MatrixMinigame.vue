<template>
  <div class="mg-wrap">
    ...
    <!-- Secuencia objetivo -->
    <div class="mg-header">
      <div class="mg-title">SINCRONIZAR REGISTRO — INTRODUCE LA SECUENCIA EN ORDEN</div>
...
function initCells() {
  ...
  cells.value = vals.map(v => ({ value: v, justCorrect: false, justWrong: false }));
}
...
function shuffleValues() {
  ...
  cells.value.forEach((c, i) => { c.value = shuffled[i]; });
}
...
function pickCell(i) {
  ...
  if (cell.value === expected) {
    ...
    setTimeout(() => { cell.justCorrect = false; }, 500);
    progress.value++;
    ...
    errors.value++;
    cell.justWrong = true;
    setTimeout(() => { cell.justWrong = false; }, 500);
...
.seq-cell.done    { background: #003300; border-color: #33cc66; color: #33cc66; }
.seq-cell.current { background: #1a0000; border-color: #cc3333; color: #cc3333; animation: seq-pulse 1s infinite; }
.seq-cell.pending { background: #0a0a0a; border-color: #1e1e1e; color: #2a2a2a; }
...
.info-label { font-size: 8px; color: #2a2a2a; letter-spacing: 1px; }
.info-val   { font-size: 14px; color: #555; font-weight: bold; margin-top: 1px; }
.info-item.warn   .info-val { color: #cc8800; animation: blink-warn 1s infinite; }
.info-item.danger .info-val { color: #cc3333; animation: blink-warn 0.6s infinite; }
...
.grid-cell:hover { background: #161616; border-color: #222; }
...
.grid-cell.correct .cell-val { color: #33cc66 !important; }
...
.grid-cell.wrong .cell-val { color: #ff4444 !important; }
...
/* ── Barra de tiempo ── */
.mg-timebar { height: 2px; background: #111; }
.mg-timefill {
  height: 100%;
  background: #cc3333;
...
.mg-timefill.low { background: #ff4444; animation: timeblink 0.5s infinite; }
@keyframes timeblink { 0%,100%{opacity:1} 50%{opacity:0.3} }
...
.res-title { font-size: 13px; color: #cc3333; letter-spacing: 2px; }
.res-sub   { font-size: 10px; color: #553333; }

.result-fade-enter-active { transition: opacity 0.2s; }
