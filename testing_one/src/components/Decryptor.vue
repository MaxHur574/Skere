<template>
    ...
    <div class="dec-app">
      ...
      <div v-if="phase === 'input'" class="dec-main">
        ...
        <div class="dec-form">
          ...
          />
          <p v-if="claveError" class="dec-error">⛔ Clave incorrecta. Formato: XXX-XXX-XXX</p>
      ...
      <div v-else-if="phase === 'glitch'" class="dec-glitch-screen">
        <div class="glitch-lines">
          <div v-for="(l, i) in glitchLines" :key="i" class="glitch-line" :class="l.type">{{ l.text }}</div>
        </div>
        <div class="glitch-bar">
          <div class="glitch-fill" :style="{ width: glitchProgress + '%' }" />
        </div>
        <div class="glitch-label">PROCESANDO... {{ Math.round(glitchProgress) }}%</div>
      </div>
      ...
      <div v-else-if="phase === 'files'" class="dec-files">
        <div class="files-header">
          ...
          <div class="files-sub">2 archivos generados — doble clic para abrir</div>
        </div>
        ...
        <div class="files-status">Sistema en espera — selecciona un archivo</div>
      </div>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showPinViewer" class="overlay" @click.self="showPinViewer = false">
        <div class="notepad-window">
          <div class="notepad-header">
            <span>📄 PIN_BOVEDA.txt — Bloc de notas</span>
            <button @click="showPinViewer = false">✕</button>
          </div>
          <pre class="notepad-body">SYSTEM://archive_note
      ...
      <div v-if="showBoveda" class="overlay" @click.self="closeBoveda">
        <div class="boveda-window" :class="{ glitching: bovedaGlitch }">
          ...
          <div v-if="bovedaPhase === 'pin'" class="boveda-body">
            ...
            <button class="bov-btn" @click="tryPin" :disabled="pinDigits.some(d => !d)">CONFIRMAR</button>
          </div>
          ...
          <div v-else-if="bovedaPhase === 'confirming'" class="boveda-body">
            ...
            <div class="confirm-bar">
              <div class="confirm-fill" :style="{ width: confirmProgress + '%' }" />
            </div>
          </div>
          ...
          <div v-else-if="bovedaPhase === 'corrupted'" class="boveda-body">
            ...
            <button class="bov-recover-btn" @click="startMinigame">[ INICIAR RECUPERACIÓN ]</button>
          </div>

          <!-- Estado: minijuego -->
          <div v-else-if="bovedaPhase === 'minigame'" class="boveda-body boveda-mg">
            <MatrixMinigame
...
function tryClave() {
  ...
  if (val === CORRECT_CLAVE) {
    ...
    setTimeout(() => { claveError.value = false; }, 2500);
  }
}
...
function onMinigameSuccess() {
  ...
  system.triggerEvent('notification', { message: 'Acceso a bóveda concedido.' });
}
...
.dec-logo { text-align: center; }
.dec-hex  {
  font-size: 44px; color: #cc3333;
  text-shadow: 0 0 20px #cc3333;
  animation: hex-pulse 2s infinite;
}
@keyframes hex-pulse {
  0%,100% { opacity:1; text-shadow: 0 0 20px #cc3333, 0 0 40px #cc3333; }
  50%      { opacity:.55; text-shadow: 0 0 8px #cc3333; }
}
.dec-title { font-size: 24px; color: #ddd; letter-spacing: 6px; font-weight: bold; margin-top: 4px; }
.dec-sub   { font-size: 10px; color: #333; letter-spacing: 2px; margin-top: 4px; }
...
.dec-form-label { font-size: 10px; color: #444; letter-spacing: 1.5px; }
...
.dec-input:focus { border-color: #cc3333; box-shadow: 0 0 10px rgba(204,51,51,.15); }
.dec-input.error { border-color: #ff4444; animation: shake 0.3s ease; }
@keyframes shake { 0%,100%{transform:translateX(0)} 25%{transform:translateX(-6px)} 75%{transform:translateX(6px)} }

.dec-error {
  font-size: 11px; color: #cc3333; margin: 0;
}
...
.dec-btn:hover {
  background: #1a0000;
  border-color: #cc3333;
  box-shadow: 0 0 12px rgba(204,51,51,.15);
}
...
.glitch-line {
  font-size: 12px;
  animation: lineIn .12s ease-out;
}
@keyframes lineIn { from{opacity:0;transform:translateX(-6px)} to{opacity:1;transform:translateX(0)} }
.glitch-line.ok   { color: #226622; }
.glitch-line.warn { color: #aa7700; }
.glitch-line.log  { color: #888; }

.glitch-bar  { height:2px; background:#1a1a1a; border-radius:2px; overflow:hidden; margin-bottom:8px; }
.glitch-fill { height:100%; background:#cc3333; box-shadow:0 0 8px #cc3333; transition:width .15s linear; }
.glitch-label { font-size:10px; color:#333; letter-spacing:2px; }
...
.files-title { font-size: 13px; color: #33cc66; font-family: 'Courier New', monospace; }
.files-sub   { font-size: 10px; color: #333; margin-top: 4px; letter-spacing:.5px; }

.files-list { flex: 1; padding: 8px; display: flex; flex-direction: column; gap: 4px; }
...
.file-row {
  ...
  animation: fileAppear .4s ease-out;
}
.file-row:nth-child(2) { animation-delay: .2s; animation-fill-mode: both; }
@keyframes fileAppear { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
.file-row:hover    { background: #111; border-color: #333; }
.file-row.selected { background: #0d1a0d; border-color: #cc3333; }

.fr-icon { font-size: 22px; flex-shrink: 0; }
.fr-name { font-size: 13px; color: #ccc; font-family: 'Courier New', monospace; }
.fr-meta { font-size: 10px; color: #444; margin-top: 2px; }
...
.notepad-header button { background:none;border:none;color:#555;cursor:pointer;font-size:13px; }
.notepad-header button:hover { color:#ff4444; }
.notepad-body {
  padding: 20px; color: #aaa;
  font-family: 'Courier New', monospace;
  font-size: 13px; white-space: pre-wrap; line-height: 1.8; margin: 0;
}
.notepad-footer {
...
.boveda-window {
  ...
  box-shadow: 0 0 40px rgba(204,51,51,.2);
  overflow: hidden;
  transition: transform .05s;
}
.boveda-window.glitching {
  animation: bov-glitch .15s steps(2) 4;
}
@keyframes bov-glitch {
  0%   { transform: translate(-3px,1px) skewX(-1deg); filter: hue-rotate(20deg); }
...
.boveda-header button { background:none;border:none;color:#553333;cursor:pointer;font-size:13px; }
.boveda-header button:hover { color:#ff4444; }
...
.boveda-mg { padding: 0; justify-content: flex-start; }

.bov-icon   { font-size: 40px; opacity: .35; }
.bov-icon-ok  { opacity: 1; color: #33cc66; font-size: 40px; }
.bov-icon-err { opacity: 1; color: #cc3333; font-size: 40px; }

.bov-title    { font-size: 14px; color: #888; letter-spacing: 2px; }
.bov-title-ok { color: #33cc66 !important; }
.bov-label    { font-size: 12px; color: #444; letter-spacing: 1px; }

.pin-row { display: flex; gap: 8px; margin: 4px 0; }
.pin-digit {
...
.pin-digit:focus { border-color: #cc3333; box-shadow: 0 0 8px rgba(204,51,51,.2); }
.pin-digit.error { border-color: #ff4444; animation: shake .3s ease; }

.bov-error { font-size: 11px; color: #cc3333; margin: 0; }

.bov-btn {
  background: #0d0000; border: 1px solid #441111; color: #cc3333;
  padding: 8px 24px; border-radius: 4px; cursor: pointer;
  font-size: 11px; letter-spacing: 2px; font-family: 'Courier New', monospace;
  transition: background .2s; margin-top: 4px;
}
.bov-btn:hover:not(:disabled) { background: #1a0000; border-color: #cc3333; }
.bov-btn:disabled { opacity: .3; cursor: default; }

.confirm-bar  { width: 200px; height: 3px; background: #1a1a1a; border-radius:2px; overflow:hidden; margin-top: 4px; }
.confirm-fill { height:100%; background: #33cc66; transition: width .04s linear; }

.bov-err-title  { font-size: 18px; color: #cc3333; font-weight: bold; letter-spacing: 3px; }
.bov-err-sub    { font-size: 12px; color: #882222; letter-spacing: 1px; }
.bov-err-divider { width: 200px; height: 1px; background: #1a0000; margin: 4px 0; }
.bov-err-text   { font-size: 12px; color: #665544; line-height: 1.8; text-align: center; }

.bov-recover-btn {
  background: #0d0000; border: 1px solid #cc4400; color: #cc6622;
  padding: 10px 20px; border-radius: 4px; cursor: pointer;
  font-family: 'Courier New', monospace; font-size: 13px; letter-spacing: 1px;
  margin-top: 8px;
  transition: all .2s;
}
.bov-recover-btn:hover { background: #1a0800; border-color: #ff6633; color: #ff8844; }

.success-date { font-size: 16px; color: #33cc66; letter-spacing: 2px; margin-top: 4px; }
.bov-open-btn {
  background: #001a00; border: 1px solid #33cc66; color: #33cc66;
...
.bov-open-btn:hover { background: #003300; box-shadow: 0 0 12px rgba(51,204,102,.2); }
</style>